//! 命令行入口（GUI 二进制 `dev-tools` 与控制台二进制 `dev-tools-cli` 共用）。
//!
//! 之所以拆出来：Windows 下 `dev-tools.exe` 用 `windows_subsystem = "windows"`，
//! PowerShell 调它会立即返回、stdout 不会被 PowerShell 接管。`dev-tools-cli.exe`
//! 走 console 子系统，专门给 PowerShell / cmd 用。两者命令集合完全一致，逻辑全部
//! 收敛在本模块。
...
#[derive(Subcommand)]
pub enum TaskBranchGroupAction {
    /// 列表查询
    List {
        #[arg(long)]
...
pub fn dispatch(command: Commands) {
    match command {
        Commands::TaskBranchGroup { action } => run_task_branch_group(action),
        Commands::Domain { action } => run_domain(action),
    }
}
...
fn open_db() -> crate::cookie_bridge::db::Db {
    let app_data_dir = default_app_data_dir();
    std::fs::create_dir_all(&app_data_dir).expect("create app data dir failed");
    let db_path = app_data_dir.join("data.db");
    crate::cookie_bridge::db::Db::open(&db_path).expect("open db failed")
}

fn run_task_branch_group(action: TaskBranchGroupAction) {
    let db = open_db();
    ...
    match action {
        ...
        TaskBranchGroupAction::Create {
            ...
            let item = crate::task_branch_group::db::CreateTaskBranchGroup {
                tb_name,
                task_id,
                branch_name,
                group_type,
        ...
        TaskBranchGroupAction::Update {
            ...
            let item = crate::task_branch_group::db::UpdateTaskBranchGroup {
                tb_name,
                task_id,
                branch_name,
                group_type,
...
fn run_domain(action: DomainAction) {
    let db = open_db();
    ...
    match action {
        ...
        DomainAction::Create {
            ...
            let item = crate::cookie_bridge::db::CreateDomain {
                domain_name,
                urls,
                description,
            };
        ...
        DomainAction::Update {
            ...
            let item = crate::cookie_bridge::db::UpdateDomain {
                domain_name,
                urls,
                description,
            };
